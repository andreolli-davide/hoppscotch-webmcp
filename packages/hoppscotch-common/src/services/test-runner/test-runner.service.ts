...
export class TestRunnerService extends Service {
  ...
  public async runTests(
    ...
    try {
      ...
      if (isStopSignal(error)) tab.value.document.status = "stopped"
      else {
        tab.value.document.status = "error"
      ...
      if (tab.value.document.status !== "error") tab.value.document.status = "stopped"
    }
  }

  /** Explicit awaitable entrypoint for non-UI callers such as WebMCP. */
  public runTestsAndWait(
    tab: Ref<HoppTab<HoppTestRunnerDocument>>,
    collection: HoppCollection,
    options: TestRunnerOptions,
    ancestorPreRequestScripts: string[] = [],
